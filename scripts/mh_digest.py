...
def fetch_doi_content(doi: str) -> str:
    ...
    try:
        headers = {
            "User-Agent": (
                "Mozilla/5.0 (compatible; womens-health-digest-bot/1.0; "
...
You are a science writer and editor for a Women's Health Research Digest, writing for a journalist audience covering women's health, wellness, and medicine.

Your readers pitch to publications like Women's Health Magazine, Prevention, AARP The Magazine, National Geographic, and similar women-focused or general health outlets.
...

{
  ...
  "summary": "2-4 sentences: what researchers did, who participated (N=X, age range if relevant, menopausal status if relevant), key finding in plain language — verified against abstract",
  "why_it_matters": "1-2 sentences of real-world significance for women specifically. Do NOT imply clinical action.",
  "caveats": "comma-separated flags present in the abstract: small sample (N<100), observational design, single-center, self-reported outcomes, short follow-up, industry funding [name], preprint, secondary analysis, no female-specific breakdown — or 'None identified'",
  ...
  "excluded": false,
  "relevance_score": 7,
  "relevance_score_reason": "One sentence explaining the score — what makes it strong or weak for women's health journalism",
  "pitch_angles": [
    {
      "publication_type": "e.g. Women's Health Magazine / Prevention / AARP / National Geographic / General health",
...
- Generate MULTIPLE pitch angles (2-3 max) only when the study genuinely fits different audiences with meaningfully different framings — for example, a menopause + cognitive function study could pitch differently to Prevention (lifestyle/symptoms angle) vs AARP (longevity angle)
- Do not generate multiple angles just to pad — if one framing covers it, use one
- Publication types to consider: Women's Health Magazine (fitness, body, sex, relationships), Prevention (disease prevention, healthy aging, diet), AARP (50+, longevity, caregiving), National Geographic (science of the body, broader biology), General health (any outlet)
...
- Flag explicitly if the study did not break down results by sex/gender — this is a significant caveat for women's health coverage
...
relevance_score rubric (1-10): start at 5, then adjust:
  ...
  +2 human subjects with women or mixed-sex sample (N>=100)
  +1 clear lifestyle/wellness hook for women
  +1 clean design (RCT, longitudinal, large cohort)
  +1 menopause, hormones, reproductive health, or women-specific condition
  ...
  Topic fit bonus: menopause, bone health, cardiovascular disease in women, mental health, weight/metabolism, sexual health, sleep, aging, cancer screening, hormones score higher
...
def send_notification(category: str, chunk_label: str, study_count: int, run_date: str):
    ...
    subject = f"Women's Health Research Digest — {run_date} | {study_count} {noun.capitalize()}"
    html = f"""<!DOCTYPE html>
<html>
<body style="font-family:Georgia,serif;max-width:500px;margin:auto;padding:24px;color:#222;">
<h2 style="color:#7c3d5e;">Women's Health Research Digest</h2>
<p><strong>{study_count} new {noun}</strong> found in
<strong>{category}{chunk_label}</strong> — {run_date}</p>
<p>
  <a href="{DASHBOARD_URL}" style="display:inline-block;background:#7c3d5e;color:white;
