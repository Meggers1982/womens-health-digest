#!/usr/bin/env python3
"""
Fitness & Sports Medicine Research Digest
...
CSV_PATH = Path(__file__).parent.parent / "data" / "Fitness & Sports Medicine.csv"
...
HUMAN_SIGNALS = {
    ...
    "athlete", "athletes", "runner", "runners", "cyclist", "cyclists",
}
...
def fetch_doi_content(doi: str) -> str:
    ...
    try:
        headers = {
            "User-Agent": (
                "Mozilla/5.0 (compatible; fitness-digest-bot/1.0; "
                "+https://github.com/meaganmorris/womens-health-digest)"
            ),
...
You are a science writer and editor for a Fitness & Sports Medicine Research Digest, writing for a journalist audience covering exercise science, sports medicine, physical performance, and active living.

Your readers pitch to publications like Runner's World, Outside, Men's Health, Women's Health, Self, ESPN The Magazine, Sports Illustrated, and similar active-lifestyle or general health outlets.
...

{
  ...
  "groundbreaking": "one or more of: Counterintuitive finding / Overturns prior research / First-in-class human study / Relevant fitness or performance finding",
  "media_coverage": "[media_note from context]",
  "summary": "2-4 sentences: what researchers did, who participated (N=X, age range, athlete type/training level if relevant), key finding in plain language — verified against abstract",
  "why_it_matters": "1-2 sentences of real-world significance for active people, athletes, coaches, or sports medicine practitioners. Do NOT imply clinical action.",
  "caveats": "comma-separated flags present in the abstract: small sample (N<100), observational design, single-center, self-reported outcomes, short follow-up, industry funding [name], preprint, secondary analysis, highly trained athletes only (may not generalize), recreational athletes only — or 'None identified'",
  ...
  "excluded": false,
  "relevance_score": 7,
  "relevance_score_reason": "One sentence explaining the score — what makes it strong or weak for fitness/sports journalism",
  "pitch_angles": [
    {
      "publication_type": "e.g. Runner's World / Outside / Men's or Women's Health / Self / ESPN / General health",
      ...
      "pitch_angle": "2-3 sentence commissioning pitch: what happened, why surprising or useful, performance or lifestyle hook",
...
- Generate MULTIPLE pitch angles (2-3 max) only when the study genuinely fits different audiences with meaningfully different framings — for example, a strength training + aging study could pitch differently to Outside (performance angle) vs AARP (longevity angle)
- Do not generate multiple angles just to pad — if one framing covers it, use one
- Publication types to consider: Runner's World (running, endurance), Outside (outdoor sports, adventure, performance), Men's Health / Women's Health (fitness, body composition, strength), Self (everyday fitness, wellness), ESPN / Sports Illustrated (elite sport, injury, performance), General health (any active-lifestyle outlet)
...
- Note if the study population is highly trained athletes vs recreational — this affects generalizability
...
- If a study is conducted entirely in animals, cell lines, or computational models: set "excluded": true, headline "EXCLUDED: animal/non-human study only", relevance_score 0, other fields empty strings

relevance_score rubric (1-10): start at 5, then adjust:
  +2 counterintuitive or overturns prior belief
  +2 human subjects with adequate sample (N>=50)
  +1 clear practical hook for active people or coaches
  +1 clean design (RCT, longitudinal, large cohort)
  +1 directly actionable finding (training, recovery, nutrition, injury prevention)
  -1 per major caveat (small N, self-report, single-center, etc.)
  -2 animal/non-human only (excluded anyway)
  -1 highly specialized population with limited generalizability
  Topic fit bonus: strength training, endurance performance, injury prevention, recovery, sleep and performance, nutrition timing, mental toughness, VO2max, body composition, aging and exercise score higher
...
"""
    ...
    prompt = (
        CLAUDE_PROMPT_TEMPLATE.replace("[media_note from context]", media_note)
        + studies_block
    )
    ...
    try:
        ...
        return [{"pmid": s["pmid"], "headline": s["title"], "journal": s["journal"],
                 ...
                 "groundbreaking": "Relevant fitness or performance finding", "media_coverage": media_note,
                 "summary": "", "why_it_matters": "", "caveats": "",
                 "fact_check_note": "", "excluded": False,
                 "relevance_score": 0, "relevance_score_reason": "",
...
def send_notification(category: str, chunk_label: str, study_count: int, run_date: str):
    noun = "study" if study_count == 1 else "studies"
    subject = f"Fitness & Sports Medicine Digest — {run_date} | {study_count} {noun.capitalize()}"
    html = f"""<!DOCTYPE html>
<html>
<body style="font-family:Georgia,serif;max-width:500px;margin:auto;padding:24px;color:#222;">
<h2 style="color:#1a4d2e;">Fitness &amp; Sports Medicine Research Digest</h2>
<p><strong>{study_count} new {noun}</strong> found in
<strong>{category}{chunk_label}</strong> — {run_date}</p>
<p>
  <a href="{DASHBOARD_URL}" style="display:inline-block;background:#1a4d2e;color:white;
  padding:10px 20px;border-radius:6px;text-decoration:none;font-size:15px;">
  View Dashboard →</a>
</p>
<p style="font-size:0.85em;color:#888;margin-top:2em;">
  Fitness &amp; Sports Medicine Research Digest · PubMed + Claude + SERPAPI
</p>
</body>
</html>"""
    try:
...
def main():
    ...
    print("=" * 60)
    print("Fitness & Sports Medicine Research Digest")
